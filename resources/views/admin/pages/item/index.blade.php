                ...
                <div class="page-header card-header">
                    <div class="row">
                        <div class="col-sm-7 col-auto">
                            <h3 class="page-title">{{ __('pages.items') }}</h3>
                        </div>
                        <div class="col-sm-5 col">
                            <a href="{{ route('item.upsert') }}" class="btn btn-primary float-end ">  <i class="ti-plus"></i> {{ __('pages.add_item') }}</a>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                        ...
                        <div class="card card-header ">
                            <div class="card-body   " >
                                <div class="table-responsive ">
                                    <form class="form" action="{{ route('item.filter') }}" method="get">
                                    ...
                                    <table id="example" class=" display  table table-hover table-center mb-0"  filter="{{ route('item.filter') }}">
                                        <thead>
                                            <tr>
                                                <th></th>
                                                <th>{{ __('pages.name') }}</th>
                                                ...
                                                <tr class="record">
                                                    ...
                                                    <td class="text-end">
                                                        <div class="actions">
                                                            ...
                                                            <a  data-bs-toggle="modal" href="#" class="btn btn-sm bg-danger-light btn_delete" route="{{ route('item.delete',['item' => $item->id])}}">
                                ...
                                <nav aria-label="Page navigation example" class="mt-2">
                                    <ul class="pagination">
                                        @for($i = 1; $i <= $items->lastPage(); $i++)
                                            <li class="page-item @if(request()->input('page') == $i) active @endif"><a class="page-link" href="?name={{request()->input('name')}}&page={{$i}}">{{$i}}</a></li>
                                        @endfor
                                    </ul>
                                </nav>
                            </div>

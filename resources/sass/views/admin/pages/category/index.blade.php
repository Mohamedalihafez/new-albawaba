                ...
                <div class="page-header card-header">
                    <div class="row">
                        <div class="col-sm-7 col-auto">
                            <h3 class="page-title">{{ __('pages.categories') }}</h3>
                        </div>
                        <div class="col-sm-5 col">
                            <a href="{{ route('category.upsert') }}" class="btn btn-primary float-end ">  <i class="ti-plus"></i> {{ __('pages.add_category') }}</a>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                        ...
                        <div class="card card-header ">
                            <div class="card-body   " >
                                <div class="table-responsive ">
                                    <form class="form" action="{{ route('category.filter') }}" method="get">
                                        <div class="form-group d-flex align-categories-center">
                                    ...
                                    <table id="example" class=" display  table table-hover table-center mb-0"  filter="{{ route('category.filter') }}">
                                        <thead>
                                            <tr>
                                                <th></th>
                                                <th>{{ __('pages.name') }}</th>
                                        ...
                                        <tbody>
                                            @foreach($categories as $category)
                                                <tr class="record">
                                                    ...
                                                    <td class="text-end">
                                                        <div class="actions">
                                                            <a href="{{ route('category.upsert',['category' => $category->id]) }}" class="btn btn-sm bg-success-light" >
                                                                <i class="ti-pencil"></i> {{ __('pages.edit') }}
                                                            </a>
                                                            <a  data-bs-toggle="modal" href="#" class="btn btn-sm bg-danger-light btn_delete" route="{{ route('category.delete',['category' => $category->id])}}">
                                ...
                                <nav aria-label="Page navigation example" class="mt-2">
                                    <ul class="pagination">
                                        @for($i = 1; $i <= $categories->lastPage(); $i++)
                                            <li class="page-category @if(request()->input('page') == $i) active @endif"><a class="page-link" href="?name={{request()->input('name')}}&page={{$i}}">{{$i}}</a></li>
                                        @endfor
                                    </ul>
                                </nav>
                            </div>

                ...
                <div class="page-header card-header">
                    <div class="row">
                        ...
                        <div class="col-sm-5 col">
                            <a href="{{ route('service.upsert') }}" class="btn btn-primary float-end ">  <i class="ti-plus"></i> إضافة خدمه</a>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                        ...
                        <div class="card card-header ">
                            <div class="card-body   " >
                                <div class="table-responsive ">
                                    <form class="form" action="{{ route('service.filter') }}" method="get">
                                        <div class="form-group d-flex align-services-center">
                                    ...
                                    <table id="example" class=" display  table table-hover table-center mb-0"  filter="{{ route('service.filter') }}">
                                        <thead>
                                            <tr>
                                                <th></th>
                                                <th>عنوان</th>
                                        ...
                                        <tbody>
                                            @foreach($services as $service)
                                                <tr class="record">
                                                    ...
                                                    <td class="text-end">
                                                        <div class="actions">
                                                            <a href="{{ route('service.upsert',['service' => $service->id]) }}" class="btn btn-sm bg-success-light" >
                                                                <i class="ti-pencil"></i> {{ __('pages.edit') }}
                                                            </a>
                                                            <a  data-bs-toggle="modal" href="#" class="btn btn-sm bg-danger-light btn_delete" route="{{ route('service.delete',['service' => $service->id])}}">
                                ...
                                <nav aria-label="Page navigation example" class="mt-2">
                                    <ul class="pagination">
                                        @for($i = 1; $i <= $services->lastPage(); $i++)
                                            <li class="page-service @if(request()->input('page') == $i) active @endif"><a class="page-link" href="?name={{request()->input('name')}}&page={{$i}}">{{$i}}</a></li>
                                        @endfor
                                    </ul>
                                </nav>
                            </div>
...
function edit_service(el) {
    var link = $(el) //refer `a` tag which is clicked
    var modal = $("#edit_service") //your modal
